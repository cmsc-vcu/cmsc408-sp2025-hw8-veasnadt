...
all_combinations AS (
    SELECT 
        region.Region,
        income_group.income_cat
    FROM 
        (SELECT DISTINCT Region FROM wdi_country_only WHERE Region IS NOT NULL) AS region
    CROSS JOIN 
        (SELECT DISTINCT `Income Group` AS income_cat FROM wdi_country_only) AS income_group
)
SELECT 
    a.Region,
    a.income_cat
FROM all_combinations a
LEFT JOIN region_income ri
  ON a.Region = ri.Region AND a.income_cat = ri.`Income Group`
WHERE ri.Region IS NULL
ORDER BY a.Region, a.income_cat;
""")
...
SELECT 
    income_category,
    country_count,
    ROUND(100.0 * country_count / total_countries, 2) AS pct_of_total
FROM base, overall_total
